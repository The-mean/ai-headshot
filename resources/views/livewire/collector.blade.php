<div class="min-h-screen bg-zinc-50 px-6 py-10 text-zinc-900 transition dark:bg-zinc-950 dark:text-zinc-100" x-data="collectorApp(@js($campaignId))" x-init="init()">
    <div class="mx-auto max-w-3xl">
        <div class="rounded-3xl border border-zinc-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/80">
            <div class="mb-6 flex items-center justify-between">
                <h1 class="text-2xl font-semibold tracking-tight">Share your experience</h1>
                <button
                    type="button"
                    class="rounded-full border border-zinc-300 px-4 py-2 text-sm dark:border-zinc-700"
                    @click="darkMode = !darkMode; syncTheme()"
                >
            ...
            <div class="relative overflow-hidden rounded-2xl bg-zinc-100 dark:bg-zinc-800">
                ...
                <div
                    x-show="countdown > 0"
                    class="absolute inset-0 flex items-center justify-center bg-black/50 text-7xl font-bold text-white"
                    x-text="countdown"
                ></div>
            </div>

            <div class="mt-6 grid gap-3 sm:grid-cols-2">
                <button class="rounded-2xl bg-zinc-900 px-4 py-3 text-sm font-medium text-white dark:bg-white dark:text-zinc-900" @click="requestCamera" :disabled="loading">
                    Enable Camera
                </button>

                <button class="rounded-2xl border border-zinc-300 px-4 py-3 text-sm font-medium dark:border-zinc-700" @click="startRecordingFlow" :disabled="!stream || recording || loading">
                    Start 3-2-1 Record
                </button>

                <button class="rounded-2xl border border-zinc-300 px-4 py-3 text-sm font-medium dark:border-zinc-700" @click="stopRecording" :disabled="!recording">
                    Stop Recording
                </button>

                <label class="cursor-pointer rounded-2xl border border-zinc-300 px-4 py-3 text-center text-sm font-medium dark:border-zinc-700">
            ...
            <div class="mt-6" x-show="uploadProgress > 0">
                <div class="mb-2 flex justify-between text-xs text-zinc-500 dark:text-zinc-400">
                    <span>Uploading...</span>
                    <span x-text="`${uploadProgress}%`"></span>
                </div>
                <div class="h-2 rounded-full bg-zinc-200 dark:bg-zinc-800">
                    <div class="h-2 rounded-full bg-zinc-900 transition-all dark:bg-white" :style="`width: ${uploadProgress}%`"></div>
                </div>
            </div>
            ...
            return {
                ...
                async requestCamera() {
                    try {
                        ...
                        this.statusText = 'Camera is ready.';
                    } catch (error) {
                ...
                startRecording() {
                    ...
                    this.mediaRecorder.onstop = () => {
                        ...
                        this.previewUrl = URL.createObjectURL(this.videoBlob);
                        this.$refs.previewVideo.load();
                        this.statusText = 'Preview ready.';
                    };
                ...
                async uploadVideo() {
                    ...
                    if (!presignResponse.ok) {
                        this.statusText = 'Presign failed.';
                        this.loading = false;
                        return;
                    }
                    ...
                    await fetch('/api/v1/testimonials', {
                        ...
                        body: JSON.stringify({
                            ...
                            meta: {
                                uploaded_via: 'collector_ui',
                                user_agent: navigator.userAgent,
                            },
                        }),
                    });

                    this.statusText = 'Uploaded successfully. Pending review.';
                    this.loading = false;
                },
